...
describe("/api/skater endpoints", () => {
  ...
  function cleanup(){
    return db.raw(
      "TRUNCATE canskate_sessions, canskate_groups, skaters RESTART IDENTITY CASCADE"
    );
  }
  ...
  context("Given there are folders and notes in the database", () => {
    ...
    describe("/api/skater route", () => {
      ...
      it("GET responds with 200 and all the skaters", async () => {
        ...
        expect(responseSkaters).to.eql(skaters);
      });
    });
    describe("/api/skater/:id route", () => {
      ...
      it("DELETE responds with 204 and deletes the skater",async () => {
        ...
        const databaseSkaters = (await supertest(app).get('/api/skater').expect(200)).body;
        expect(deletedSkaters).to.eql(databaseSkaters.map(skater=>({...skater,birthdate: new Date(skater.birthdate).toLocaleDateString()})));
      });
    });
  });
  context(`'skaters' database is empty`, () => {
    describe("/api/skater route", () => {
      ...
      it('POST creates a skater, responding with 201 and the new skater', async ()=>{
        ...
        const res = await supertest(app).post('/api/skater').send(skater).expect(201);
        expect(res.body.fullname).to.eql(skater.fullname);
        expect(res.body.gender).to.eql(skater.gender);
        expect(new Date(res.body.birthdate).toLocaleDateString()).to.eql(skater.birthdate)
